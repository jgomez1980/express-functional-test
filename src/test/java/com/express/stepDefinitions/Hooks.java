package com.express.stepDefinitions;
...
import com.express.framework.web.TestContext;
...
public class Hooks {
	...
	public void tearDown(Scenario scenario) {
		if (scenario.getSourceTagNames().contains("@NoBrowser")) {
			System.out.println("Ending service tests");
		} else {
			if (driver != null) {
	...
	protected static void takeScreenShot(Scenario scenario) {
		if (scenario.isFailed()) {
			String screenShotName = scenario.getName().replaceAll("\\s", "").replaceAll("[^\\p{ASCII}]", "")
			...
			try {
				...
				Reporter.addStepLog("Was not possible to take screenshot");
			}
		}
	}
